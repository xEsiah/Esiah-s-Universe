...
class InfiniteGridMenu {
  ...
  // --- CORRECTION CLÉ : Gestionnaire d'images robuste ---
  _initTexture() {
    const gl = this.gl;
    ...
    Promise.all(
      this.items.map(
        (item) =>
          new Promise((resolve) => {
            ...
            img.onerror = () => {
              // Fallback : On génère un carré gris si l'image plante
      ...
      images.forEach((img, i) => {
        ...
        // Mode "Cover" pour éviter la déformation
...
export default function InfiniteMenu({ items = [], scale = 1.0, onItemClick }) {
  ...
  useEffect(() => {
    ...
    // --- CORRECTION CLÉ : Observer pour le redimensionnement immédiat ---
      ...
      {activeItem && (
        <div className="infinite-menu-overlay">
          ...
          <div
            ...
            className={`action-button cursor-target ${isMoving ? "inactive" : "active"}`}
          >
            <span className="action-button-text">Learn more</span>
