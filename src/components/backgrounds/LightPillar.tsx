...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  useEffect(() => {
    ...
    const effectiveQuality =
      isMobile && quality === "high" ? "medium" : quality;
    ...
    const qualitySettings: Record<"low" | "medium" | "high", QualityConfig> = {
      low: {
        iterations: 24,
        waveIterations: 1,
        pixelRatio: 0.5,
    ...
    const animate = (currentTime: number) => {
      ...
      if (deltaTime >= 16.6) {
        ...
        lastTime = currentTime - (deltaTime % 16.6);
      }
  ...
  if (!webGLSupported)
    return (
      <div
        className={`w-full h-full absolute inset-0 flex items-center justify-center bg-black/10 text-[#888] text-[14px] ${className}`}
        style={{ mixBlendMode }}
      >
        WebGL not supported
      </div>
  ...
  return (
    <div
      ref={containerRef}
      className={`w-full h-full absolute inset-0 transition-opacity duration-1000 ${className}`}
      style={{
        mixBlendMode,
        pointerEvents: "none",
        opacity: opacity,
